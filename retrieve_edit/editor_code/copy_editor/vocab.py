...
def flat(L):
    rt = []
    for i in L:
        if isinstance(i, list): rt.extend(flat(i))
        else: rt.append(i)
    return rt
...
def base_plus_copy_indices(words, dynamic_vocabs, base_vocab, volatile=False):
    ...
    try:
        ...
        diff = max(base_indices.mask.size(1) - copy_indices.mask.size(1),
                   copy_indices.mask.size(1) - base_indices.mask.size(1))
        if base_indices.mask.size(1) < copy_indices.mask.size(1):
            zer = torch.zeros((base_indices.mask.size(0), diff), dtype=torch.long)

            base_indices = SequenceBatch(torch.cat((base_indices.values,
                                                    torch.zeros((base_indices.values.size(0), diff),
                                                                dtype=base_indices.values.dtype,
                                                                device=base_indices.values.device)), dim=1),
                                         torch.cat((base_indices.mask,
                                                    torch.zeros((base_indices.mask.size(0), diff),
                                                                dtype=base_indices.mask.dtype,
                                                                device=base_indices.mask.device)), dim=1))
        else:
            zer = torch.zeros((copy_indices.mask.size(0), diff), dtype=torch.long)

            copy_indices = SequenceBatch(torch.cat((copy_indices.values,
                                                    torch.zeros((copy_indices.values.size(0), diff),
                                                                dtype =copy_indices.values.dtype,
                                                                device = copy_indices.values.device)), dim=1),
                                         torch.cat((copy_indices.mask,
                                                    torch.zeros((copy_indices.mask.size(0), diff),
                                                                dtype =copy_indices.mask.dtype,
                                                                device = copy_indices.mask.device)), dim = 1))
