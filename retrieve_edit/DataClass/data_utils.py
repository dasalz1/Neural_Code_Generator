...
def create_vocab_dictionary(path, file, save=True, filter_count=500):
    ...
    try:
        ...
        tokens_dict = pickle.load(open('./all_tokens.pickle', 'rb'))
...
def tokenize(s,
    ...
    if keep_literal:
        ...
        literals = ["(?<=[^a-zA-Z0-9]){}|^{}".format(re.escape(l), re.escape(l)) for l in numbers]
...
def rand_select(list, k):
    print("Randomly selected {} examples from {} examples:".format(k, len(list)))
...
def to_string_opt(opt):
    ...
    """
    s = []
    s.append('u_{}'.format(opt.unit))
    s.append('d_metric_x_{}'.format(opt.distance_metric_x))
    s.append('d_thre_x_{}'.format(opt.distance_threshold_x))
    s.append('d_metric_y_{}'.format(opt.distance_metric_y))
    s.append('d_thre_y_{}'.format(opt.distance_threshold_y))
    s.append('n_{}'.format(opt.n_leftmost_tokens))
    s.append('max_can_{}'.format(opt.max_num_candidates))
    return '__'.join(s)
...
def get_urls_from_github(num_repos):
    ...
    urls = urls[:num_repos]
    print("[.] Collected {} URLs.".format(len(urls)))
    return urls
...
def get_urls_from_csv(path_csv):
    ...
    print('Read {} urls from csv file'.format(len(urls)))
    return urls
