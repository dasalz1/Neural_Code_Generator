...
class TrainState(object):
    ...
    def save(self, path):
        ...
        torch.save(check_point, join(path, 'checkpoint.pt'))
        torch.save(self.model.state_dict(), join(path, 'model'))
        torch.save(self.optimizer.state_dict(), join(path, 'optimizer'))
    ...
    def load(cls, path, model, optimizer):
        ...
        if torch.cuda.is_available():
            # load_fxn = lambda path: torch.load(path, map_location='cpu')
            load_fxn = torch.load
        else:
        ...
        # load model
        try:
            checkpoint = torch.load(join(path, 'checkpoint.pt'))
        except:
            print 'Unable to load checkpoint.pt'
        try:
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        except Exception as e:
            print e
            try:
                print 'retrying to load optimizer params'
